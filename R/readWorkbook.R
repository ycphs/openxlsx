...
read.xlsx.default <- function(xlsxFile,
  ...
  if (!is.null(namedRegion)) {
    ...
    if (grepl(":", region, fixed = TRUE)) {
      ...
      cols <- seq(from = cols[1],
                  to = cols[2],
                  by = 1)
      rows <- seq(from = rows[1],
                  to = rows[2],
                  by = 1)
  ...
  ## get file
  worksheet <-
    xmlFiles[grepl(pattern = tolower(sheet),
                   x = tolower(xmlFiles),
                   fixed = TRUE)]
  ...
  if (fillMergedCells & length(cell_info$cellMerge) > 0) {
    ...
    cell_info$string_refs <-
      cell_info$string_refs[order(as.integer(gsub(
        "[A-Z]", "", cell_info$string_refs, perl = TRUE
      )),
  ...
  if (detectDates) {
    ...
    dateStyleIds <-
      which(sapply(xf, function(x)
        grepl(lookingFor, x), USE.NAMES = FALSE)) - 1L
    ...
    v[isDate] <-
      format(as.Date(as.integer(v[isDate]) - origin, origin = "1970-01-01"),
             "%Y-%m-%d")
