...
test_that("Empty workbook", {
  ...
  x <- read.xlsx(wb)
  xf<-read.xlsx(wb,asdatatable = F)
  expect_equal(nrow(x), 0)  
  expect_equal(nrow(xf), 0) 
  
  
  expect_equal(names(x), "a")  
  expect_equal(names(xf), "a")  
  
  x <- read.xlsx(wb, sheet = 1, colNames = FALSE)
  expect_equal(data.table("X1" = "a", stringsAsFactors = FALSE), x)  
  
  x <- read.xlsx(wb, sheet = 1, colNames = FALSE,asdatatable = F)
  ...
  x <- read.xlsx(wb, sheet = 1, colNames = TRUE,asdatatable = F)
  expect_equal(nrow(x), 0)  
  expect_equal(names(x), "a")
  
  x <- read.xlsx(wb, sheet = 1, colNames = FALSE, skipEmptyRows = FALSE)
  expect_equal(data.table("X1" = "a", stringsAsFactors = FALSE), x)  
  x <- read.xlsx(wb, sheet = 1, colNames = FALSE, skipEmptyRows = FALSE,asdatatable = F)
  expect_equal(data.frame("X1" = "a", stringsAsFactors = FALSE), x)  
...
test_that("Reading NAs and NaN values", {
  ...
  expect_equal(read.xlsx(fileName,asdatatable = FALSE), expected_df)
  
  expected_dt <- as.data.table(structure(list(X = c(NA_real_, NA_real_, NA_real_)
                                , Y = c("a", "b", "c")
                                , Z = c(NA, 99, NA)
                                , Z2 = c(1, NA, NA))
                           , .Names = c("X", "Y", "Z", "Z2")
  ...
  expect_equal(read.xlsx(fileName), expected_dt)
  ...
  expect_equal(read.xlsx(wb,asdatatable = FALSE), expected_df)
  
  expected_df <- as.data.table(structure(list(X = c(NA_real_, NA_real_, NA_real_)
                                , Y = c("a", "b", "c")
                                , Z = c(NA, 99, NA)
                                , Z2 = c(1, NA, NA))
                           , .Names = c("X", "Y", "Z", "Z2")
  ...
  expect_equal(read.xlsx(wb,asdatatable = TRUE), expected_df)
  ...
  expect_equal(b, read.xlsx(wb,asdatatable = FALSE))  
  expect_equal(data.table(b), read.xlsx(wb,asdatatable = TRUE))  
  expect_equal(b, read.xlsx(wb,asdatatable = FALSE))  
  expect_equal(data.table(b), read.xlsx(fileName,asdatatable = TRUE))  
  ...
  expect_equal(b, read.xlsx(wb, sheet = 2,asdatatable = FALSE))  
  expect_equal(b, read.xlsx(fileName, sheet = 2,asdatatable = FALSE)) 
  expect_equal(data.table(b), read.xlsx(wb, sheet = 2))  
  expect_equal(data.table(b), read.xlsx(fileName, sheet = 2)) 
...
test_that("Reading from new workbook 2 ", {
  ...
  class(dt$Currency) <- "currency"
  class(dt$Accounting) <- "accounting"
  class(dt$hLink) <- "hyperlink"
  class(dt$Percentage) <- "percentage"
  class(dt$TinyNumber) <- "scientific"
  ...
  xdf <- read.xlsx(wb, sheet = 1, detectDates = FALSE, asdatatable = FALSE)
  xdf[[1]] <- convertToDate(xdf[[1]])
  expect_equal(object = xdf, expected = genDf(), check.attributes = FALSE)
  
  xdf <- read.xlsx(wb, sheet = 1, detectDates = FALSE, asdatatable = TRUE)
  xdf[[1]] <- convertToDate(xdf[[1]])
  expect_equal(object = xdf, expected = genDt(), check.attributes = FALSE)
  
  
  x <- read.xlsx(wb, sheet = 1, detectDates = TRUE, asdatatable = FALSE)
  expect_equal(object = x, expected = genDf(), check.attributes = FALSE)
  
  
  x <- read.xlsx(wb, sheet = 1, detectDates = TRUE , asdatatable = TRUE)
  expect_equal(object = x, expected = genDt(), check.attributes = FALSE)
...
test_that("Reading from new workbook cols/rows DF", {
  ...
  cols <- 1:3
  rows <- 1:10
  x <- read.xlsx(wb, 1, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE)
  y <- read.xlsx(tempFile, 1, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE)
  
  
  
  df <- data.frame(mtcars)
  df<-df[sort((rows-1)[(rows-1) <= nrow(mtcars)]), sort(cols[cols <= ncol(mtcars)])]
  ...
  cols <- 1:300
  rows <- 1:1000
  x <- read.xlsx(wb, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE)
  y <- read.xlsx(tempFile, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE)
  ...
  cols <- 1:3
  rows <- 12:13
  x <- suppressWarnings(read.xlsx(wb, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE))
  y <- suppressWarnings(read.xlsx(tempFile, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE))
  ...
  x <- read.xlsx(wb, sheet = 3, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE)
  y <- read.xlsx(tempFile, sheet = 3, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = FALSE)
  
  df <- data.frame(mtcars)[sort((rows-1)[(rows-1) <= nrow(mtcars)]), sort(cols[cols <= ncol(mtcars)])]
  rownames(df) <- 1:nrow(df)
  ...
  cols <- c(1, 6, 12)
  rows <- seq(1, 31, by = 2)
  x <- read.xlsx(wb, sheet = 4, colNames = TRUE, rowNames = TRUE, rows = rows, cols = cols, asdatatable = FALSE)  
  y <- read.xlsx(tempFile, sheet = 4, colNames = TRUE, rowNames = TRUE, rows = rows, cols = cols, asdatatable = FALSE)  
  
  df <- data.frame(mtcars)[sort((rows-1)[(rows-1) <= nrow(mtcars)]),cols[-1]-1]
  expect_equal(object = x, expected = y, check.attributes = FALSE)
  expect_equal(object = df, expected = x, check.attributes = FALSE)
...
test_that("Reading from new workbook cols/rows DF", {
  ...
  cols <- 1:3
  rows <- 1:10
  x <- read.xlsx(wb, 1, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE)
  y <- read.xlsx(tempFile, 1, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE)
  ...
  cols <- 1:300
  rows <- 1:1000
  x <- read.xlsx(wb, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE)
  y <- read.xlsx(tempFile, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE)
  ...
  cols <- 1:3
  rows <- 12:13
  x <- suppressWarnings(read.xlsx(wb, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE))
  y <- suppressWarnings(read.xlsx(tempFile, sheet = 2, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE))
  ...
  x <- read.xlsx(wb, sheet = 3, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE)
  y <- read.xlsx(tempFile, sheet = 3, colNames = TRUE, rowNames = FALSE, rows = rows, cols = cols, asdatatable = TRUE)
  ...
  cols <- c(1, 6, 12)
  rows <- seq(1, 31, by = 2)
  x <- read.xlsx(wb, sheet = 4, colNames = TRUE, rowNames = TRUE, rows = rows, cols = cols, asdatatable = TRUE)  
  y <- read.xlsx(tempFile, sheet = 4, colNames = TRUE, rowNames = TRUE, rows = rows, cols = cols, asdatatable = TRUE)  
