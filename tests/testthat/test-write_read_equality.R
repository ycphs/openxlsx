...
test_that("Writing then reading returns identical data.frame 1", {
  ...
  x <- read.xlsx(xlsxFile = fileName, detectDates = TRUE, asdatatable = FALSE)
...
test_that("Writing then reading returns identical data.frame 2", {
  ...
  xNoDateDetection <- read.xlsx(xlsxFile = fileName, detectDates = FALSE,asdatatable = FALSE)  
...
test_that("Writing then reading rowNames, colNames combinations", {
  ...
  x <- read.xlsx(fileName, sheet = 1, rowNames = TRUE,asdatatable = FALSE)
  attributes(x)$row.names<-as.integer(attributes(x)$row.names)
  expect_equal(object = x, expected = as.data.frame(mtcars), check.attributes = TRUE)
  
  
  ## rowNames = TRUE, colNames = FALSE
  write.xlsx(mtcars, file = fileName, overwrite = TRUE, rowNames = TRUE, colNames = FALSE)
  x <- read.xlsx(fileName, sheet = 1, rowNames = TRUE, colNames = FALSE,asdatatable = FALSE)
  attributes(x)$row.names<-as.integer(attributes(x)$row.names)
  ...
  x <- read.xlsx(fileName, sheet = 1, rowNames = FALSE, colNames = TRUE,asdatatable = FALSE)
  expect_equal(object = x, expected = mtcars, check.attributes = FALSE)
  
  ## rowNames = FALSE, colNames = FALSE
  write.xlsx(mtcars, file = fileName, overwrite = TRUE, rowNames = FALSE, colNames = FALSE)
  x <- read.xlsx(fileName, sheet = 1, rowNames = FALSE, colNames = FALSE,asdatatable = FALSE)
  expect_equal(object = x, expected = mtcars, check.attributes = FALSE)
...
test_that("Writing then reading returns identical data.frame 3", {
  ...
  rows <- 1:4
  cols <- c(1, 3, 5)
  x <- read.xlsx(xlsxFile = fileName, detectDates = TRUE, rows = rows, cols = cols,asdatatable = FALSE)
...
test_that("Writing then reading returns identical data.table 1", {
  ...
  x <- read.xlsx(xlsxFile = fileName, detectDates = TRUE, asdatatable = TRUE)
  expect_equal(object = x, expected = genDf(), check.attributes = FALSE)
...
test_that("Writing then reading returns identical data.table 2", {
  
  curr_wd <- getwd()
  
  ## data.frame of dates
  dates <- data.table("d1" = Sys.Date() - 0:500)
  for(i in 1:3) dates <- cbind(dates, dates)
  names(dates) <- paste0("d", 1:8)
  ...
  xNoDateDetection <- read.xlsx(xlsxFile = fileName, detectDates = FALSE,asdatatable = TRUE)  
  dateOrigin <- getDateOrigin(fileName)
...
test_that("Writing then reading rowNames, colNames combinations DT", {
  ...
  x <- read.xlsx(fileName, sheet = 1, rowNames = TRUE,asdatatable = TRUE)
  attributes(x)$row.names<-as.integer(attributes(x)$row.names)
  ...
  ## rowNames = TRUE, colNames = FALSE
  write.xlsx(mtcars, file = fileName, overwrite = TRUE, rowNames = TRUE, colNames = FALSE)
  x <- read.xlsx(fileName, sheet = 1, rowNames = TRUE, colNames = FALSE,asdatatable = TRUE)
  attributes(x)$row.names<-as.integer(attributes(x)$row.names)
  expect_equal(object = x, expected = mtcars, check.attributes = FALSE)
  expect_equal(object = rownames(x), expected = rownames(mtcars))
  ...
  ## rowNames = FALSE, colNames = TRUE
  write.xlsx(mtcars, file = fileName, overwrite = TRUE, rowNames = FALSE, colNames = TRUE)
  x <- read.xlsx(fileName, sheet = 1, rowNames = FALSE, colNames = TRUE,asdatatable = TRUE)
  expect_equal(object = x, expected = mtcars, check.attributes = FALSE)
  
  ## rowNames = FALSE, colNames = FALSE
  write.xlsx(mtcars, file = fileName, overwrite = TRUE, rowNames = FALSE, colNames = FALSE)
  x <- read.xlsx(fileName, sheet = 1, rowNames = FALSE, colNames = FALSE,asdatatable = TRUE)
  expect_equal(object = x, expected = mtcars, check.attributes = FALSE)
...
test_that("Writing then reading returns identical data.table 3", {
  ...
  rows <- 1:4
  cols <- c(1, 3, 5)
  x <- read.xlsx(xlsxFile = fileName, detectDates = TRUE, rows = rows, cols = cols,asdatatable = TRUE)
  expect_equal(object = x, expected = genDf()[sort((rows-1)[(rows-1) <= nrow(df)]), sort(cols[cols <= ncol(df)]),with=FALSE], check.attributes = FALSE)
...
test_that("Writing then reading returns identical data.table 4", {
